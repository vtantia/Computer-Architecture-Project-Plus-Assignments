...
# Which parameters to use for:
# G_WIDTH, G_HASH_SIZE, S_WIDTH, S_HASH_SIZE, FH_HASH_SIZE
# Select at least one
defines = [
...
# Generates the command to launch the code in a new tmux session and
# log all the information to named files.
def command(prog, ff, cmd, cnt):
  st = "cd ~/spec_2006/" + prog + "; date > prog.date-" + cnt + "; ~/pin-3.0-76991-gcc-linux/pin -t ~/pin-3.0-76991-gcc-linux/source/tools/Project/obj-ia32/Project.so -o " + prog + ".output-" + cnt + " -f " + str(ff) + " -- " + cmd + " > " + prog + ".stdout-" + cnt + " 2> prog.error-" + cnt + "; date >> prog.date-" + cnt
  return "bash -c '" + st + "'"
