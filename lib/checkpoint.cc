 ...
 *  export license.
 *
 *  $Id: checkpoint.cc,v 1.1.1.1 2006/05/23 13:53:59 mainakc Exp $
 *
 *************************************************************************/
...
CheckPoint::CheckPoint (FILE *f, count_t tm, int ready, ChkType type)
  : SimObject (ready)
#else
  CheckPoint::CheckPoint (FILE *f, count_t tm, int ready, ChkType type) : SimObject (ready)
#endif
{
  fp = f;
  chkpoint_time = tm;
...
UnCheckPoint::UnCheckPoint(FILE *f, int ready) : SimObject (ready)
{
  fp = f; 
  UndoNumbering (); 
  H = createIntHashtable (32);
}

UnCheckPoint::~UnCheckPoint ()
{
  releaseIntHashtable (H);
}
...
void UnCheckPoint::MainLoop (void)
{
  ...
  for (s = SimObject::obj_list; s; s = s->next) {
    if (s->my_id == -1) continue;
    Assert (!(e = associateIntHashtable (H, s->my_id)), "Duplicate id's!");
    e = addIntHashtable (H, s->my_id);
    e->value.p = s;
#if defined (SYNCHRONOUS)
    s->t->tasklist = NULL;
#endif
  ...
  printf ("RESTORE complete, simulation time = %llu\n", 
	  (unsigned long long)get_time ()/2);
  /* I'm done */
}
 ...
 *
 */
SimObject *UnCheckPoint::ObjectPointer (int id)
{
  struct IntHashelement *e;

  Assert (e = associateIntHashtable (H, id), "Inconsistent id; probably restoring from wrong checkpoint.");
  return (SimObject *)e->value.p;
}
