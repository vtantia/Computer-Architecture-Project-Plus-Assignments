...
#ifdef __ia64__
void context_destroy (context_t *p)
#else
...
task *
create_task (void (*func)(void), unsigned stacksize, char *name)
{
   char*     ptr;
   task*     tptr;
...
task *
create_task (void (*func)(void *), void *arg, unsigned stacksize, char *name)
{
   char*     ptr;
   task*     tptr;
   unsigned  size = sizeof(task);
   ...
   tptr->c.sz = stacksize;
#ifdef __ia64__
   context_init (&tptr->c,stub_function);
#else
