 ...
 *  export license.
 *
 *  $Id: config.cc,v 1.1.1.1 2006/05/23 13:53:59 mainakc Exp $
 *
 *************************************************************************/
#include <stdio.h>
#include <string.h>
...
static struct helem *gethelem (char *name)
{
  struct Hashlistelement *e;
  struct helem *h;
  
  e = associateHashtable (H, name);
  if (!e) {
    e = addHashtable (H, name);
    h = newhelem ();
    e->value.p = (char*)h;
  }
  else {
    h = (struct helem *)e->value.p;
  }
  return h;
}
 ...
 *
 */
void RegisterDefault (char *name, LL i)
{
  struct Hashlistelement *e;
  struct helem *h;
...
void RegisterDefault (char *name, int i)
{
  struct Hashlistelement *e;
  struct helem *h;
...
void RegisterDefault (char *name, float f)
{
  struct Hashlistelement *e;
  struct helem *h;
...
void OverrideConfig (char *name, char *s)
{
  struct Hashlistelement *e;
  struct helem *h;
