...
UINT64 insMemReadOpCount[MAX_MEM_OPS];  // instruction memory read operand
                                        // count
UINT64 insMemWriteOpCount[MAX_MEM_OPS]; // instruction memory write operand
                                        // count
UINT64 insOpCount[MAX_INS_OPS];         // instruction operand count
UINT64 insRegReadOpCount[MAX_INS_OPS];  // instruction register source
                                        // operand count
UINT64 insRegWriteOpCount[MAX_INS_OPS]; // instruction register destination
                                        // operand count

UINT32 maxBytesTouched =
    0; // maximum number of bytes touched by an instruction
UINT64 totalBytesTouched =
    0; // total number of bytes touched by all instructions
INT32 maxImmediate = 0;            // maximum value of immediate field
INT32 minImmediate = 0xFFFFFFFFLL; // minimum value of immediate field
ADDRDELTA maxDisplacement =
    0; // maximum value of displacement in memory addressing
...
KNOB<UINT64>
    KnobFastForward(KNOB_MODE_WRITEONCE, "pintool", "f", "0",
                    "number of instructions to fast forward in billions");
...
VOID InsCount(void) { icount++; }

ADDRINT FastForward(void) {
  return (icount >= fastForwardIns && icount < maxIns);
}

ADDRINT Terminate(void) { return (icount >= maxIns); }
...
VOID RecordIns(ADDRINT insAddr, UINT32 insSize, UINT32 opCount,
               UINT32 opRCount, UINT32 opWCount, INT32 maxImm,
               INT32 minImm) {
  insOpCount[opCount]++;
  insRegReadOpCount[opRCount]++;
  insRegWriteOpCount[opWCount]++;
  insSizeCount[insSize]++;
  if (maxImm > maxImmediate)
    maxImmediate = maxImm;
...
VOID RecordInsType1(INS_CATEGORY category, INS_CATEGORY memOp1Category,
                    ADDRINT memOp1Addr, UINT32 memOp1Size,
                    UINT32 readOpCount, UINT32 writeOpCount,
                    ADDRDELTA maxdisp, ADDRDELTA mindisp) {
  ...
  if (memOp1Size > maxBytesTouched)
    maxBytesTouched = memOp1Size;
  ...
  if (mindisp < minDisplacement)
    minDisplacement = mindisp;
}

VOID RecordInsType2(INS_CATEGORY category, INS_CATEGORY memOp1Category,
                    ADDRINT memOp1Addr, UINT32 memOp1Size,
                    INS_CATEGORY memOp2Category, ADDRINT memOp2Addr,
                    UINT32 memOp2Size, UINT32 readOpCount,
                    UINT32 writeOpCount, ADDRDELTA maxdisp,
                    ADDRDELTA mindisp) {
...
VOID RecordInsType3(INS_CATEGORY category, INS_CATEGORY memOp1Category,
                    ADDRINT memOp1Addr, UINT32 memOp1Size,
                    INS_CATEGORY memOp2Category, ADDRINT memOp2Addr,
                    UINT32 memOp2Size, INS_CATEGORY memOp3Category,
...
VOID RecordInsType4(INS_CATEGORY category, INS_CATEGORY memOp1Category,
                    ADDRINT memOp1Addr, UINT32 memOp1Size,
                    INS_CATEGORY memOp2Category, ADDRINT memOp2Addr,
                    UINT32 memOp2Size, INS_CATEGORY memOp3Category,
                    ADDRINT memOp3Addr, UINT32 memOp3Size,
                    INS_CATEGORY memOp4Category, ADDRINT memOp4Addr,
                    UINT32 memOp4Size, UINT32 readOpCount,
                    UINT32 writeOpCount, ADDRDELTA maxdisp,
                    ADDRDELTA mindisp) {
...
/* Array containing pointers to analysis functions for collecting traces.
 * Different
 * analysis functions are called depending on the number of memory
 * operands. Currently,
 * instructions with upto four memory operands are supported. I do not
 * thinks x86 has
 * instructions with more than 4 memory operands. */
AFUNPTR RecordInsTypeFuncs[] = {
    AFUNPTR(RecordInsType0), AFUNPTR(RecordInsType1),
...
VOID Instruction(INS ins, VOID *v) {
  ...
  ASSERT(INS_Size(ins) < MAX_INS_SIZE,
         "Instruction size exceeded MAX_INS_SIZE!");
  ...
  for (UINT32 Op = 0; Op < ops; Op++) {
    ...
    if (INS_OperandIsMemory(ins, Op)) {
      ...
      if (dispValue > maxDisp)
        maxDisp = dispValue;
      if (dispValue < minDisp)
        minDisp = dispValue;
    }
  }
  INS_InsertIfCall(ins, IPOINT_BEFORE, (AFUNPTR)FastForward, IARG_END);
  INS_InsertThenCall(
      ins, IPOINT_BEFORE, (AFUNPTR)RecordIns, IARG_ADDRINT,
      INS_Address(ins),           // instruction address
      IARG_UINT32, INS_Size(ins), // instruction size
      IARG_UINT32, ops,           // instruction operand count
      IARG_UINT32,
      INS_MaxNumRRegs(ins), // instruction register read operand count
      IARG_UINT32,
      INS_MaxNumWRegs(ins), // instruction register write operand count
  ...
  case XED_CATEGORY_CALL:
    if (INS_IsDirectCall(ins))
      category = INS_DIRECT_CALL;
    else
      category = INS_INDIRECT_CALL;
    break;
  ...
  ASSERT(category > INS_STORE && category < INS_COUNT,
         "Invalid Ins Category!");
  ...
  if (memOps) {
    for (UINT32 memOp = 0; memOp < memOps; memOp++) {
      if (INS_MemoryOperandIsRead(ins, memOp)) {
        ...
        IARGLIST_AddArguments(
            args, IARG_UINT32, INS_LOAD, // instruction type load
            IARG_MEMORYOP_EA, memOp,     // Effective memory address
            IARG_UINT32,
            INS_MemoryOperandSize(ins, memOp), // memory access size
            IARG_END);
      }
      ...
      if (INS_MemoryOperandIsWritten(ins, memOp)) {
        ...
        IARGLIST_AddArguments(
            args, IARG_UINT32, INS_STORE, // instruction type store
            IARG_MEMORYOP_EA, memOp,      // Effective memory address
            IARG_UINT32,
            INS_MemoryOperandSize(ins, memOp), // memory access size
            IARG_END);
    ...
    IARGLIST_AddArguments(args, IARG_UINT32, effectiveReadMemOps,
                          IARG_UINT32, effectiveWriteMemOps, IARG_ADDRINT,
                          maxDisp, IARG_ADDRINT, minDisp, IARG_END);
  }
  ASSERT(effectiveMemOps < MAX_MEM_OPS,
         "Number of effective memory operands exceeded MAX_MEM_OPS!");

  INS_InsertIfCall(ins, IPOINT_BEFORE, (AFUNPTR)FastForward, IARG_END);
  INS_InsertThenPredicatedCall(
      ins, IPOINT_BEFORE, (AFUNPTR)RecordInsTypeFuncs[effectiveMemOps],
...
int main(int argc, char *argv[]) {
  ...
  for (UINT32 i = 0; i < MAX_MEM_OPS; i++)
    insMemOpCount[i] = 0;
  for (UINT32 i = 0; i < MAX_MEM_OPS; i++)
    insMemReadOpCount[i] = 0;
  for (UINT32 i = 0; i < MAX_MEM_OPS; i++)
    insMemWriteOpCount[i] = 0;
  for (UINT32 i = 0; i < MAX_INS_OPS; i++)
    insOpCount[i] = 0;
  for (UINT32 i = 0; i < MAX_INS_OPS; i++)
    insRegReadOpCount[i] = 0;
  for (UINT32 i = 0; i < MAX_INS_OPS; i++)
    insRegWriteOpCount[i] = 0;
